...
export function writable(isWritable: boolean) {
    return function (constrOrProto: any, methodName: string, descriptor: PropertyDescriptor): PropertyDescriptor {
        // console.log(constrOrProto);
...
export function logParameter(constrOrProto: any, methodName: string, index: number)  {
    const key = `${methodName}_decor_params_indexes`;
    const proto = typeof constrOrProto === 'function' ? constrOrProto.prototype : constrOrProto;

    (proto[key] ??=[]).push(index);
}

export function logMethod(constrOrProto: any, methodName: string, descriptor: PropertyDescriptor): PropertyDescriptor {
    const originalMethod = descriptor.value;
